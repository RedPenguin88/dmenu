...
drawmenu(void)
{
	...
	if (lines > 0) {
		...
		x += w;
		for (item = curr; item != next; item = item->right)
			x = drawitem(item, x, 0, textw_clamp(item->text, mw - x - TEXTW(">") - TEXTW(numbers) - border_width));
		if (next) {
			w = TEXTW(">");
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_text(drw, mw - w - TEXTW(numbers) - border_width, 0, w, bh, lrpad / 2, ">", 0);
		}
	}
	drw_setscheme(drw, scheme[SchemeNorm]);
...
usage(void)
{
	fputs("usage: dmenu [-bcfsFvxP] [-l lines] [-p prompt] [-fn font] [-m monitor]\n"
	      "             [-nb color] [-nf color] [-sb color] [-sf color]\n"
	      "             [-nhb color] [-nhf color] [-shb color] [-shf color] [-w windowid] [-bw width]"
		  "\n", stderr);
	exit(1);
}
...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
		...
		if (!strcmp(argv[i], "-v")) {      /* prints version information */
			...
			fast = 1;
		else if (!strcmp(argv[i], "-F"))   /* toggle fuzzy matching */
			fuzzy = 0;
			...
			fstrncmp = strncmp;
			fstrstr = strstr;
		} else if (!strcmp(argv[i], "-P")) /* toggle password mode */
			passwd = 1;
